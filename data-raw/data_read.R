## Fetch and prepare airport data from OpenFlights database
## This script should be run periodically to update airport data
...
message("Fetching airport data from OpenFlights...")
airports <- tryCatch(
  readr::read_csv(
    "https://raw.githubusercontent.com/jpatokal/openflights/master/data/airports.dat",
    col_names = airport_cols,
    show_col_types = FALSE
  ),
  error = function(e) {
    stop("Failed to fetch airport data: ", e$message)
  }
)

# Fetch ISO country codes
message("Fetching ISO country codes...")
iso2 <- tryCatch(
  readr::read_csv(
  ...
  error = function(e) {
    stop("Failed to fetch ISO-2 codes: ", e$message)
  }
)
...
iso3 <- tryCatch(
  ...
  error = function(e) {
    stop("Failed to fetch ISO-3 codes: ", e$message)
  }
)
...
message("Saving airport data to data/airports.rda...")
save(airports, file = "data/airports.rda", compress = "xz")

message("Data update complete!")
message("Total airports: ", nrow(airports))
message("Last updated: ", Sys.Date())
